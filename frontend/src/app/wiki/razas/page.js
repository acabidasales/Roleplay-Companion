            ...
            {data.map(item => {
                return (
                    <div key={item.id}>
                        ...
                        <img alt={"raza "+item.id} src={"/iconos_razas/"+item.imagen} className='mx-auto lg:w-[125px]'/>
                        ...
                        {item.virtudes.map(virtud => {
                            return (
                                <div key={virtud.id} className="flex flex-col items-center bg-bg-950 border border-sky-800 rounded-lg shadow md:flex-row md:max-w-lg hover:bg-bg-950">
                                    <img alt={"vidrud "+virtud.id} className="object-cover w-full rounded-t-lg h-96 md:h-auto md:w-1/3 md:rounded-none md:rounded-s-lg" src={"/iconos_razas/virtudes/"+virtud.imagen}/>
                        ...
                        {item.defectos.map(defecto => {
                            return (
                                <div key={defecto.id} className="flex flex-col items-center bg-bg-950 border border-sky-800 rounded-lg shadow md:flex-row md:max-w-lg hover:bg-bg-950">
                                    <img alt={"defecto "+defecto.id} className="object-cover w-full rounded-t-lg h-96 md:h-auto md:w-1/3 md:rounded-none md:rounded-s-lg" src={"/iconos_razas/defectos/"+defecto.imagen}/>
                                    <div className="flex flex-col justify-between p-4 leading-normal">
